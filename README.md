...
> 该项目其实是一个 skill，它的作用是针对一个需求，在自测完、联调前，结合业务对代码 diff 先进行一次 AI 审查

**流程：**
1. 当前分支与 master 分支的 diff
2. 技术性审查：包括但不限于空指针、资源泄露、并发问题、性能问题等
3. 业务性审查：PRD、后端设计文档、分功能模块审查
4. 问题分类：P0（阻塞流程/数据错误）、P1（不阻塞的问题）、P2（建议/优化）
5. 修复建议：问题代码位置、问题原因 及 修复建议
6. 输出报告
